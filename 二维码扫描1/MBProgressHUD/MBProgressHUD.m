...
+ (instancetype)showHUDAddedTo:(UIView *)view offset:(CGFloat)height animated:(BOOL)animated {
    MBProgressHUD *hud = [[self alloc] initWithView:view offset:height];
    hud.removeFromSuperViewOnHide = YES;
    [view addSubview:hud];
    [hud showAnimated:animated];
    return hud;
...
- (instancetype)initWithView:(UIView *)view offset:(CGFloat)height {
    NSAssert(view, @"View must not be nil.");
    return [self initWithFrame:CGRectMake(view.x, view.y + height, view.width, view.height - height)];
}
